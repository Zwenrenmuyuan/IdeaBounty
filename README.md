...
商业点子收集器 MVP。后端已实现本地账号会话、投稿处理、AI 评估、Embedding、查重、红包估值、管理员处理和模拟打款。React 前端已实现注册登录、个人投稿、信息补充、无效投稿删除、结果详情、失败重试、匹配点子脱敏摘要和管理员操作闭环。

单机 MVP 上线步骤见[部署指南](./部署指南.md)。
...
用户可以提交原文、补充 AI 指出的缺失信息、删除尚未形成有效结果的投稿，并查看处理状态、五维评分、查重结果、红包估值和模拟打款状态；管理员可以通过 `/admin` 查看汇总和可审核投稿并执行一次性最终处理。
