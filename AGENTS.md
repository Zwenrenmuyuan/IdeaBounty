...
Copy `backend/.env.example` to `.env`; never commit credentials, raw tokens, or machine paths. Compose credentials are local-only. Derive ownership from the authenticated user; never accept client-provided `user_id`. Only ordinary users may mutate submissions; administrators are restricted to review operations. Do not expose password/token hashes, internal IDs, or content hashes.
