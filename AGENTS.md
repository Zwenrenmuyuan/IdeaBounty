...
Backend code lives in `backend/src/idea_bounty/`: `api/` exposes FastAPI routes and dependencies, `services/` owns transactions, `schemas/` defines Pydantic boundaries, `models/` contains SQLAlchemy entities, and `db/` owns persistence. Migrations are in `backend/alembic/`; tests are in `backend/tests/`. Root `compose.yaml` starts PostgreSQL using `infra/postgres/`. Treat `技术方案.md` as the approved architecture source. Frontend, AI processing, deduplication, and scoring are not implemented yet.
